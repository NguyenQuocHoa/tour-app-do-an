...
export default function Destination() {
	...
	useEffect(async () => {
		...
		let res = await API.get(`${endpoints["destination_all"]}${query}`);
	...
	const update_tag_tour = async tourID => {
		let res = await API.post(
			endpoints["tour_total_update_tag"](tourID),
			{
				tag1: tag_info
			},
			{
	...
	const add_tag_tour = async tourID => {
		let res = await API.post(
			endpoints["tour_total_add_tag"](tourID),
			{
				tag: tag_info
			},
			{
	...
	const add_departure = async e => {
		...
		try {
			...
			formdata.append("image", image.current.files[0]);
			formdata.append("active", active);
			let res = await API.post(endpoints["destination_all"], formdata, {
				headers: {
					Authorization: `Bearer ${cookie.load("access_token")}`
				}
			});
	...
	const del_tour = async tourID => {
		try {
			let res = await API.delete(
				endpoints["tour_total_by_id"](tourID),
				{
					headers: {
						Authorization: `Bearer ${cookie.load("access_token")}`
					}
	...
	if (tour !== null && tour !== "" && tour !== undefined) {
		if (user.length !== 0 && user.is_superuser == true) {
			info = info_btn(openMOdal);
			del = del_btn(del_tour_selected);
			btn_add = (
				<div className="group_search">
					<div className="label_search">Add Departure</div>
				...
				<Container className="py-5">
						...
						<div class="container">
							<div class="row ">
								<div class="col-md-12 heading-section text-center">
									...
									<h2 class="under-subheading mb-4">
										Tour Departure 1
									</h2>
								</div>
							</div>
					...
					{/* <div className="list_tour row">
						<div className="search_field_tour col-md-3 col-sm-4">
							...
							<div className="cate_tour"></div>
							<form
								className="form_search_tour p-2"
								onKeyDown={e => {
									if (e.key === "Enter") {
										history.push(
											`/departure/?name=${name_search}`
										);
									}
								}}
							...
							>
								...
								<div className="group_search">
									<div className="label_search">Category</div>
									<div className="list_btn">
										{tag.map((u, index) => (
											<TagMap
												key={index}
						...
						<div className="col-md-9 col-sm-8">
							...
							>
								...
								<Col md={3} xs={6} className="py-3"></Col>
							</div>
						</div>
						<Pagination
	...
	return (
		<>
			<Col md={4} className="py-3">
				<div
					className="slick_img"
					style={{ backgroundImage: `url(${props.tours.image})` }}
					id={props.tours.id}
				>
					...
					<div className="br_name">
						<div className="name_tour_br">{props.tours.name}</div>
					</div>

					<div className="option_wrapper">
						<div className="overlay-bl"></div>
						<a href={path} className="src_detail"></a>
		...
		<>
			<div className="">
				<label
					...
					className="form-control"
					onClick={e =>
						props.history.push(`/departure/?tag=${props.tag.name}`)
					}
				>
					{props.tag.name}
				</label>
